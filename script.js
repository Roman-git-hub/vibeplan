// VibePlan Sync Edition - client-side app with manual GitHub sync
const CREDENTIALS = { user: "Roman", pass: "12345678" };
const STATE_KEY = "vibeplan_state_v2";
const SETTINGS_KEY = "vibeplan_settings_v2";
...
function loadState(){ const raw=localStorage.getItem(STATE_KEY); if(raw) state=JSON.parse(raw); else { state=defaultIdeas(); saveState(); } }
function saveState(){ localStorage.setItem(STATE_KEY, JSON.stringify(state)); }
function loadSettings(){ const raw=localStorage.getItem(SETTINGS_KEY); if(raw) settings=JSON.parse(raw); else settings={ ghOwner:'', ghRepo:'', ghPath:'data.json', ghToken:'' }; }
...
function onPlatformToggle(e){ const idx=parseInt(e.target.dataset.idx); const plat=e.target.dataset.plat; if(typeof idx!=='number' || !state.items[idx]) return; state.items[idx].platforms[plat]=e.target.checked; saveState(); }

function onActionClick(e){ const action=e.target.dataset.action; const idx=parseInt(e.target.dataset.idx); if(action==='togglePub'){ state.items[idx].published=!state.items[idx].published; saveState(); renderWeek(); } else if(action==='edit'){ openNewModal(state.items[idx]); } else if(action==='del'){ if(confirm('Удалить идею?')){ state.items.splice(idx,1); saveState(); renderWeek(); } } }

function showModal(id){ $(id).classList.add('show'); } function hideModal(id){ $(id).classList.remove('show'); }
function initUI(){ $('loginBtn').onclick=onLogin; $('btnLogout').onclick=onLogout; $('btnNew').onclick=()=>openNewModal(); $('btnExport').onclick=exportTXT; $('btnSettings').onclick=()=>showSettings(); $('btnSync').onclick=syncNow; $('saveSettings').onclick=saveSettingsUI; $('testConn').onclick=testConnection; $('closeSettings').onclick=()=>hideModal('settingsModal'); $('closeNew').onclick=()=>hideModal('newModal'); $('saveIdea').onclick=saveIdeaFromModal; $('loginUser').value=CREDENTIALS.user; }

function onLogin(){ const u=$('loginUser').value.trim(); const p=$('loginPass').value; if(u===CREDENTIALS.user && p===CREDENTIALS.pass){ hideModal('loginModal'); loadState(); loadSettings(); renderWeek(); } else alert('Неверный логин или пароль'); }
...
function showSettings(){ showModal('settingsModal'); loadSettings(); $('ghOwner').value=settings.ghOwner||''; $('ghRepo').value=settings.ghRepo||''; $('ghPath').value=settings.ghPath||'data.json'; $('ghToken').value=settings.ghToken||''; }
function saveSettingsUI(){ settings.ghOwner=$('ghOwner').value.trim(); settings.ghRepo=$('ghRepo').value.trim(); settings.ghPath=$('ghPath').value.trim()||'data.json'; settings.ghToken=$('ghToken').value.trim(); saveSettings(); hideModal('settingsModal'); alert('Настройки сохранены локально. Нажмите «Синхронизировать сейчас», чтобы записать в GitHub.'); }
function testConnection(){ if(!settings.ghOwner||!settings.ghRepo||!settings.ghToken){ alert('Заполните owner, repo и token в настройках.'); return; } const api=`https://api.github.com/repos/${settings.ghOwner}/${settings.ghRepo}`; fetch(api,{headers:{Authorization:'token '+settings.ghToken}}).then(r=>{ if(r.ok) alert('Соединение установлено.'); else r.text().then(t=>alert('Ошибка: '+r.status+' '+t)); }).catch(e=>alert('Ошибка: '+e.message)); }

async function syncNow(){ const statusEl=$('syncStatus'); statusEl.textContent='Статус: синхронизация...'; if(!settings.ghOwner||!settings.ghRepo||!settings.ghToken){ alert('Заполните настройки GitHub.'); showSettings(); statusEl.textContent='Статус: ошибка: настройки не заполнены'; return; } const apiBase=`https://api.github.com/repos/${settings.ghOwner}/${settings.ghRepo}/contents/${settings.ghPath}`; try{ const resGet=await fetch(apiBase,{headers:{Authorization:'token '+settings.ghToken}}); let sha=null; if(resGet.ok){ const j=await resGet.json(); sha=j.sha; } const content=btoa(unescape(encodeURIComponent(JSON.stringify(state,null,2)))); const body={message:'Update VibePlan data',content}; if(sha) body.sha=sha; const resPut=await fetch(apiBase,{method:'PUT',headers:{Authorization:'token '+settings.ghToken,'Content-Type':'application/json'},body:JSON.stringify(body)}); if(resPut.ok){ statusEl.textContent='Статус: ✅ синхронизировано'; alert('Синхронизировано с GitHub.'); } else { const t=await resPut.text(); statusEl.textContent='Статус: ❌ ошибка при записи'; alert('Ошибка записи: '+resPut.status+' '+t); } }catch(e){ statusEl.textContent='Статус: ❌ ошибка сети'; alert('Ошибка синхронизации: '+e.message); } }

// storage
function loadState(){ const raw=localStorage.getItem(STATE_KEY); if(raw) state=JSON.parse(raw); else { state=defaultIdeas(); saveState(); } }
function loadSettings(){ const raw=localStorage.getItem(SETTINGS_KEY); if(raw) settings=JSON.parse(raw); else settings={ ghOwner:'', ghRepo:'', ghPath:'data.json', ghToken:'' }; }
function saveSettings(){ localStorage.setItem(SETTINGS_KEY,JSON.stringify(settings)); }
function start(){ initUI(); loadState(); loadSettings(); showModal('loginModal'); renderWeek(); }
start();
